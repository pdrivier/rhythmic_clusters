...
for r = 1:length(list_of_rats)
    ...
    for w = 1:length(unique_wires)
        ...
        if n_appearances > 1
            ...
            unique_sessions = unique([subwire{:,2}]);
            days_distance = diff(unique_sessions);
            unit_letter_distance = diff([subwire{:,4}]);
            
            if days_distance < min_accept_distance && unit_letter_distance == 1
                %if the wire recorded two units whose letter designations
                %is just a single character apart, then grab only the
                %session with the highest firing rate, otherwise it's dicey
